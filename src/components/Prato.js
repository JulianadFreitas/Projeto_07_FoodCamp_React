...
export default function Prato(props) {
    ...
    {num:"dois", src:"imagens/camarao.png", alt:"prato com camarao", titulo:"Camarão", subtitulo:"Acompanha molho especial", preco:"R$ 32,90"},
   ...
   const teste4 = props.teste3;
   console.log(teste4);
   const prato = props.pratos;
   console.log(prato);
    return ( <>
        <div class="nome-tipo">Primeiro, seu prato</div>
        <div class="tipo primeira">
        { pratos.map( (item) =>  (
             <CadaPrato prato = {prato} teste5={teste4} src={item.src} alt = {item.alt} titulo ={item.titulo} subtitulo={item.subtitulo} preco={item.preco} /> 
            ))}
  </div></> 
   )
}
